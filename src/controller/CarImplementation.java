...
public class CarImplementation implements CarInterface {
    ...
    public boolean leftLaneDetect() throws DetectException{
        QueryResult q1 = checkQuery(0);
        QueryResult q2 = checkQuery(1);
    ...
    public CarPosition getRandomCar() {
        return randomCar;
    }

    public int getRandomCarPosition(){
        return this.randomCar.streetPosition;
    }
     ...
     */
    public QueryResult checkQuery(int counterSensor){
        int [] sensorData = new int[4];
        sensorData[0] =sensor1.setSensorData(counterSensor);
        sensorData[1] =sensor2.setSensorData(counterSensor);
        sensorData[2] =sensor3.setSensorData(counterSensor);
        sensorData[3] =sensor4.setSensorData(counterSensor);
    ...
    public int moveForwardRandomCar() {
        if(this.randomCar.getPosition() >=96 || this.randomCar.getPosition() < 0){  // TC1  checks if the car is out of range
            return this.randomCar.getPosition();
        } else {
            //   car.move();         // TC0      moves the car when its within range
            randomCar.move();
            return this.randomCar.getPosition();
